...
(function (MyBuilder) {
    ...
    class Point {
        ...
        constructor(...arg) {
            ...
            let temp = arg[0];
            if (temp !== undefined)
                if (temp instanceof Point || temp instanceof MyBuilder.Vector) {
                    this.x = temp.x;
                    this.y = temp.y;
                }
                else {
...
(function (MyBuilder) {
    ...
    class Vector {
        ...
        constructor(...arg) {
            ...
            let temp = arg[0];
            if (temp !== undefined)
                if (temp instanceof Vector || temp instanceof MyBuilder.Point) {
                    this.x = temp.x;
                    this.y = temp.y;
                }
                else {
    ...
    class Circle {
        ...
        constructor(...arg) {
            ...
            if (temp !== undefined)
                ...
                else if (temp instanceof MyBuilder.Point || temp instanceof MyBuilder.Vector) {
                    this.x = temp.x;
                    this.y = temp.y;
                    // @ts-ignore
...
(function (MyBuilder) {
    ...
    })(buttonList = MyBuilder.buttonList || (MyBuilder.buttonList = {}));
})(MyBuilder || (MyBuilder = {}));
var MyBuilder;
(function (MyBuilder) {
    /**
    ...
    })(cursorStyle = MyBuilder.cursorStyle || (MyBuilder.cursorStyle = {}));
})(MyBuilder || (MyBuilder = {}));
var MyBuilder;
(function (MyBuilder) {
    /**
    ...
    class Input {
        ...
        _$beforeUpdate() {
            ...
            Input._$mousePos = new MyBuilder.Vector(Input._$mousePosTemp);
            Input._$mouseGlobalPos = new MyBuilder.Vector(Input._$mouseGlobalPosTemp);
        ...
        static set cursorStyle(style) {
            ...
            MyBuilder.World.divNode.style.cursor = style;
        }
        /** 获取鼠标指针点击时样式 */
        ...
        static setCursorImage(url, xOffset = 0, yOffset = 0) {
            ...
            MyBuilder.World.divNode.style.cursor = Input._$cursorStyle;
        }
        /** 设置鼠标指针点击时图片 */
    ...
    Input._$keyUpTemp = [];
    /** 鼠标的位置,临时储存 */
    Input._$mousePosTemp = MyBuilder.Vector.zero;
    /** 鼠标的全局位置,临时储存 */
    Input._$mouseGlobalPosTemp = MyBuilder.Vector.zero;
    ...
    Input._$keyUp = [];
    /** 鼠标的位置 */
    Input._$mousePos = MyBuilder.Vector.zero;
    /** 鼠标的全局位置 */
    Input._$mouseGlobalPos = MyBuilder.Vector.zero;
    /** 一直在按下的mouseButton */
    Input._$buttonAction = [];
    /** 第一次按下的mouseButton */
    Input._$buttonDown = [];
...
(function (MyBuilder) {
    ...
    })(keyList = MyBuilder.keyList || (MyBuilder.keyList = {}));
})(MyBuilder || (MyBuilder = {}));
var MyBuilder;
(function (MyBuilder) {
    /**
    ...
    class World {
        ...
        static Init(divNode, x, y, width, height, zIndex) {
            ...
            if (World._$isInit)
                return;
            World._$isInit = true;
            World._$canvas = new MyBuilder.Canvas(new MyBuilder.Rectangle(y, y, width, height));
            ...
            World._$input = new MyBuilder.Input();
            World._$worldTree = new MyBuilder.WorldTree();
            //创建主线程
            World._$thread = new MyBuilder.Thread(60);
            //执行线程方法
            ...
            document.onkeydown = (e) => {
                MyBuilder.Input.keyDown(e.keyCode);
            };
            /** 当用户键盘按松开调用 */
            document.onkeyup = (e) => {
                MyBuilder.Input.keyUp(e.keyCode);
            };
            /** 当用户鼠标移动时调用,局部坐标 */
            divNode.onmousemove = (e) => {
                MyBuilder.Input._$setMousePos(e.offsetX, e.offsetY);
            };
            /** 当用户鼠标移动时调用,全局坐标坐标 */
            document.onmousemove = (e) => {
                MyBuilder.Input._$setMouseGlobalPos(e.screenX, e.screenY);
            };
            /** 当用户鼠标按钮按下时调用 */
            divNode.onmousedown = (e) => {
                World.divNode.style.cursor = MyBuilder.Input.cursorCheckStyle;
                MyBuilder.Input.buttonDown(e.button);
            };
            /** 当用户鼠标按钮松开时调用 */
            divNode.onmouseup = (e) => {
                World.divNode.style.cursor = MyBuilder.Input.cursorStyle;
                MyBuilder.Input.buttonUp(e.button);
            };
            /** 单用户鼠标滚轮滚动时调用 */
            // @ts-ignore
            divNode.onmousewheel = (e) => {
                MyBuilder.Input._$setMouseWheel(e.wheelDelta > 0 ? 1 : -1);
            };
        }
        /** 获取画布 */
        static get canvas() {
        ...
        static updateFunc(delta) {
            ...
            //调用移除索引子节点方法
            MyBuilder.Tree._$callRemoveAllNode();
            //调用离开方法
            MyBuilder.Tree._$callLeaveNode();
            //初始化节点
            MyBuilder.Tree._$callInitNode();
            //更新按键
...
(function (MyBuilder) {
    ...
    class Brush {
        ...
        /** 设置画笔颜色 */
        setColor(color) {
            if (color instanceof MyBuilder.Color)
                color = color.toRgba();
            // @ts-ignore
        ...
        drawCircle(position, radius) {
            ...
            this._$context2D.arc(position.x, position.y, radius, 0, MyBuilder.Utils.toRadians(360));
...
(function (MyBuilder) {
    ...
    class NodeBase {
        ...
        constructor(name) {
            ...
            this._$name = "";
            /** 节点相对于父节点的坐标 */
            this._$position = MyBuilder.Vector.zero;
            /** 节点相对于父节点的旋转角度 */
            this._$rotation = 0;
            /** 节点相对于父节点的缩放比 */
            this._$scale = MyBuilder.Vector.one;
            /** 节点相对于父节点的绘制透明度 */
            this._$alpha = 1;
            /** 节点相对于场景根节点的坐标 */
            this._$globalPosition = MyBuilder.Vector.zero;
    ...
    class Sprite extends Node2D {
        constructor(name) {
            super(name);
            /** 混合的颜色 */
            this._$blend = new MyBuilder.Color();
            /** 精灵是否居中显示,默认false */
            this._$centered = false;
            /** 精灵绘制偏移 */
            this._$offset = MyBuilder.Vector.zero;
            /** 是否启用精灵显示区域,默认false */
            this._$regionEnable = false;
            /** 精灵显示区域 */
            this._$regionRect = new MyBuilder.Rectangle();
            /** 垂直帧数,必须大于0 */
            this._$vFrames = 1;
            /** 水平帧数,必须大于0 */
            this._$hFrames = 1;
